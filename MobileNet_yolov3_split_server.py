...
class YOLOv3_server(object):
    ...
    def net(self, inputs):
        ...
        scale = self.scale
        blocks = []
        tmp = self.conv_bn_(inputs[3], 3, int(32 * scale), 2, 1)
        ...
        # yolo detector
        for i, block in enumerate(inputs[0:3]):
            if i > 0:
...
def get_yolo_server(is_tiny, class_num, anchors, anchor_mask):
    """
    ...
    if is_tiny:
        ...
        return YOLOv3_server(class_num, anchors, anchor_mask)
