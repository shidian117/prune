...
from MobileNet_yolov3_split_server import get_yolo_server
...
def freeze_model(score_threshold):
    ...
    model = get_yolo_server(ues_tiny, train_parameters['class_dim'], yolo_config['anchors'], yolo_config['anchor_mask'])
    image4 = fluid.layers.data(name='image4', shape=yolo_config['input_size'], dtype='float32')
    image1 = fluid.layers.data(name='image1', shape=[-1, 1024, 14, 14], dtype='float32')
    image2 = fluid.layers.data(name='image2', shape=[-1, 512, 28, 28], dtype='float32')
    image3 = fluid.layers.data(name='image3', shape=[-1, 256, 56, 56], dtype='float32')
    image = [image1, image2, image3, image4]
    #image = fluid.layers.data(name='image', shape=[-1, 32, 224, 224], dtype='float32')
    ...
    fluid.io.save_inference_model(train_parameters['freeze_split_dir'], ['image1','image2','image3', 'image_shape'], pred, exe, freeze_program,model_filename='__model1__', params_filename='__params1__')
    #fluid.io.save_inference_model(train_parameters['freeze_split_dir'], [], outputs, exe, freeze_program,model_filename='__model__', params_filename='__params__')
